...
def main():
    ...
    if args.map:
        ...
        if os.path.isfile(map_file):
            cspect_args.append(f"-map={map_file}.patched")
    ...
    try:
        ...
        if args.echo:
            ...
            # Closing the CSpect window (especially under mono) often returns
            # SIGSEGV (-11 / 245). That is not a launch failure.
            process.wait()
            print(f"{Colors.GREEN}CSpect closed.{Colors.RESET}")
            sys.exit(0)
        else:
