...
"""
# v10 Many changes! Hope they work ,,, 23-08-2026
...
parser.add_argument('--nex-only', action='store_true', help='Rebuild NEX from existing .bin without compiling')
...
class ParserState:
    ...
    nonex: bool = False
    org: int = "32768"
    end: str = None             # '!end=  first unused address (default $10000)
    head_file: str = None
    ...
    filetype: str = None       # File type: 'bas' or 'asm'
    ...
    cmds_: list = field(default_factory=list)
    hdf_: str = ""
    copy_hdf: str = None
    sp: int = None
    pc: int = None
    ...
    # ZX Next banked code (CODEBANK). None means "leave the compiler default".
    codewindow: str = None      # '!codewindow=   address of the code window
    codewindowsize: str = None  # '!codewindowsize= 8192 (one MMU slot) or 16384 (two)
    codebankbase: str = None    # '!codebank=     physical 8K page for CODEBANK 1
    codebankpages: str = None   # '!codebankpages= explicit page list, e.g. 40,41
    codebankdepth: str = None   # '!codebankdepth= max cross-bank call nesting
# --- END NEW ---
...
def setup_environment():
    ...
    if not os.path.exists(zxbasic_dir):
        found = []
        try:
            found = sorted(
                name for name in os.listdir(BASE_DIR)
                if name.lower().startswith("zxbasic")
                and os.path.isdir(os.path.join(BASE_DIR, name))
            )
        except OSError:
            found = []
        print(f"Error: ZX BASIC not found: {zxbasic_dir}")
        print(f"Set ZXBASIC= in {os.path.join(SCRIPTS_DIR, 'nextbuild.config')} to the compiler folder.")
        if found:
            print(f"Folders at ROOT: {', '.join(found)}")
        else:
            print("No zxbasic* folder in the NextBuild root. Copy the compiler next to Scripts/.")
        sys.exit(1)
...
from nexbuild import (  # noqa: E402
    CreateNEXFile as _nex_create_file,
    ParseNEXCfg as _nex_parse_cfg,
    ParseNEXCfgASM as _nex_parse_cfg_asm,
    parse_number,
)
...
try:
    import map_formatter # type: ignore
except ImportError:
    # If map_formatter is not found, create a dummy module
    class DummyMapFormatter:
        def main(self, *args, **kwargs):
            print("Warning: map_formatter module not found. Map formatting not supported.")
            return None

    import sys
    sys.modules['map_formatter'] = DummyMapFormatter()
...
global filename_path,filename_extension,filename_noextension,copy,gentape,makebin,binfile,pcadd,sysvars, arch, no_nex, check_cmd, cmds_
...
state.heap = DEFAULT_HEAP # a '!heap= directive in the source overrides this below
state.optimize = DEFAULT_OPTIMIZE # likewise '!opt=
state.org = DEFAULT_ORG # likewise '!org=
# --- END NEW ---

# show info
if args.quiet == 0:
    print(f"{GREEN}==============================================================================================={RESET}")
    print(f"NextBuild v10     : David Saphier / em00k - 23-AUG-2026      https://github.com/em00k/NextBuild")
...
# Directive patterns will be created dynamically in parse_directives based on file type

# procs

def CheckForSupportedFile():
    # Check if the file has a supported extension (.bas or .asm)
    ...
    try:
        ...
        print(f"{RED}Error: State object not properly initialized before CheckForSupportedFile.{RESET}")
        sys.exit(1)

    if extension == '.bas':
        state.filetype = 'bas'
        print(f"{GREEN}Detected BASIC file{RESET}")
    elif extension == '.asm':
        state.filetype = 'asm'
        print(f"{GREEN}Detected Assembly file{RESET}")
    else:
        print(f"{RED}Unsupported file type '{extension}' - ERROR{RESET}")
        print(f"{RED}Supported types: .bas (ZX Basic) and .asm (Assembly){RESET}")
        sys.exit(1)

    # Check against the basename stored in state for BASIC files
    if state.filetype == 'bas' and state.input_basename.lower() == "nextlib.bas":
...
_MODULE_BIN_RE = re.compile(r'^module(\d+)\.bin$', re.IGNORECASE)


def _is_module_bin(name):
    """True for Module000.bin through Module255.bin, not modules.bin."""
    match = _MODULE_BIN_RE.fullmatch(name)
    return bool(match and 0 <= int(match.group(1)) <= 255)


def _has_module_files(state):
    """Check if the project has Module000–Module255.bin files in the data directory."""
    local_data_dir = os.path.join(state.input_dir, 'data')
    if not os.path.isdir(local_data_dir):
        return False

    for item in os.listdir(local_data_dir):
        if _is_module_bin(item):
            return True
    return False
...
def _run_hdf_copy(src_abs, hdf_dest):
    ...
    try:
        ...
        print(f" {GREEN}- {os.path.basename(src_abs)} -> {hdf_dest}{RESET}")
        return True
...
def _sync_data_subdir(local_dir, hdf_dir):
    """Copy a data/ subdirectory tree onto the HDF image (no extra /data prefix)."""
    _ensure_hdf_dir(hdf_dir)
    synced = []
    for name in os.listdir(local_dir):
        src = os.path.join(local_dir, name)
        dest = f"{hdf_dir}/{name}"
        if os.path.isdir(src):
            synced.extend(_sync_data_subdir(src, dest))
        elif os.path.isfile(src):
            if _run_hdf_copy(src, dest):
                synced.append(dest)
    return synced


def _sync_module_files(state, target_hdf_dir_clean):
    """Sync ModuleNNN.bin and data/ subfolders onto the HDF dir, not {dir}/data."""
    local_data_dir = os.path.join(state.input_dir, 'data')
    if not os.path.isdir(local_data_dir):
    ...
    for item in os.listdir(local_data_dir):
        ...
        if os.path.isfile(local_item_path):
            if _is_module_bin(item):
                hdf_item_path = f"{target_hdf_dir_clean}/{item}"
                if _run_hdf_copy(local_item_path, hdf_item_path):
                    synced.append(item)
        elif os.path.isdir(local_item_path):
            synced.extend(_sync_data_subdir(local_item_path, f"{target_hdf_dir_clean}/{item}"))

    if synced:
        print(f"{GREEN} - Synced {len(synced)} files to {target_hdf_dir_clean}{RESET}")
    else:
        print(f"{YELLOW} - No Module000–255.bin files or data subfolders to sync{RESET}")

def _sync_selective_files(state, target_hdf_dir_clean, selected_files):
    """Sync only specified files from local data/ into the HDF dir (no /data prefix)."""
    ...
    for filename in selected_files:
        ...
        if os.path.isfile(local_item_path):
            hdf_item_path = f"{target_hdf_dir_clean}/{os.path.basename(filename)}"
            if _run_hdf_copy(local_item_path, hdf_item_path):
...
def _sync_all_data_files(state, target_hdf_dir_clean):
    """Copy everything in local data/ onto the HDF dir, not {dir}/data."""
    local_data_dir = os.path.join(state.input_dir, 'data')
    if not os.path.isdir(local_data_dir):
        print(f"{YELLOW} - Local 'data' directory not found, skipping data sync: {local_data_dir}{RESET}")
        return

    print(f"{YELLOW}Syncing files from {local_data_dir} to HDF:{target_hdf_dir_clean}{RESET}")
    synced = _sync_data_subdir(local_data_dir, target_hdf_dir_clean)
    print(f"{GREEN} - Synced {len(synced)} files to {target_hdf_dir_clean}{RESET}")
...
def ParseNEXCfg():
    """Generate a NEX .cfg from the compiled project (implemented in nexbuild.py)."""
    _nex_parse_cfg(state, BASE_DIR)


def ParseNEXCfgASM():
    """Generate a minimal NEX .cfg for assembly files (implemented in nexbuild.py)."""
    _nex_parse_cfg_asm(state, BASE_DIR)


def CreateNEXFile():
    """Package the .nex via nextcreator (implemented in nexbuild.py)."""
    # show_done=False: the success line is printed below with the build time on
    # it, so "All done" really is the last thing on screen.
    _nex_create_file(state, BASE_DIR, show_done=False)


def build_nex_from_state():
    """Rebuild cfg + nex from existing binaries; used by --nex-only."""
    if state.filetype == 'asm':
        ParseNEXCfgASM()
    else:
        ParseNEXCfg()
    if not state.module:
...
def _tidy_directive(text, prefix):
    """Normalise the spacing in a header directive, so that

        '! codebank = 80      '!heap =2048      '! nosys

    read the same as '!codebank=80, '!heap=2048 and '!nosys. Only the first
    directive on the line is touched, and only the key and its '=' - anything
    after the '=' (paths, exe command lines, nb=(...) arguments) is left alone.
    """
    esc = re.escape(prefix)
    text = re.sub(r"(" + esc + r")\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*",
                  lambda m: m.group(1) + m.group(2) + "=", text, count=1)
    return re.sub(r"(" + esc + r")\s+(?=[A-Za-z_])", r"\1", text, count=1)
...
def parse_directives(state):
    ...
    if state.filetype == 'asm':
        print(f"{YELLOW}Processing precompiler settings from .asm header!{RESET}")
        # For assembly files, look for directives in ; comments instead of ' comments
        directive_prefix = ';!'
    else:
        print(f"{YELLOW}Processing precompiler settings from .bas header!{RESET}")
        directive_prefix = "'!"

    # Create directive patterns based on the file type
    directive_patterns = {
        'org': re.compile(r"{}org={}".format(re.escape(directive_prefix), number_pattern)),
        'end': re.compile(r"{}end={}".format(re.escape(directive_prefix), number_pattern)),
        'heap': re.compile(r"{}heap={}".format(re.escape(directive_prefix), number_pattern)),
        'optimize': re.compile(r"{}opt={}".format(re.escape(directive_prefix), number_pattern)),
        'module': re.compile(r"{}module=(\w+)".format(re.escape(directive_prefix))),
        'noemu': re.compile(r"{}noemu".format(re.escape(directive_prefix))),
        'nonex': re.compile(r"{}nonex".format(re.escape(directive_prefix))),
        'docopy': re.compile(r"{}copy=(.+)".format(re.escape(directive_prefix))),
        'doasm': re.compile(r"{}$".format(re.escape(directive_prefix + 'asm'))),
        'nosys': re.compile(r"{}$".format(re.escape(directive_prefix + 'nosys'))),
        'nosp': re.compile(r"{}$".format(re.escape(directive_prefix + 'nosp'))),
        'pc': re.compile(r"{}pc={}".format(re.escape(directive_prefix), number_pattern)),
        'sp': re.compile(r"{}sp={}".format(re.escape(directive_prefix), number_pattern)),
        'bmp': re.compile(r"{}bmp=(.+)".format(re.escape(directive_prefix))),
        'bin': re.compile(r"{}bin=(\w+)".format(re.escape(directive_prefix))),
        'hdf': re.compile(r"{}hdf=(\w+)".format(re.escape(directive_prefix))),
        'origin': re.compile(r"{}origin=(.+)".format(re.escape(directive_prefix))),
        'exe': re.compile(r"{}exe=(.+)".format(re.escape(directive_prefix))),
        'nb': re.compile(r"{}nb=([a-zA-Z0-9_]+)(?:\(([^)]*)\))?".format(re.escape(directive_prefix))),
        'headerless': re.compile(r"{}headerless".format(re.escape(directive_prefix))),
        # ZX Next banked code (CODEBANK)
        'codewindowsize': re.compile(r"{}codewindowsize={}".format(re.escape(directive_prefix), number_pattern)),
        'codewindow': re.compile(r"{}codewindow={}".format(re.escape(directive_prefix), number_pattern)),
        'codebankbase': re.compile(r"{}codebank={}".format(re.escape(directive_prefix), number_pattern)),
        'codebankpages': re.compile(r"{}codebankpages=([0-9a-fx$,\s]+)".format(re.escape(directive_prefix))),
        'codebankdepth': re.compile(r"{}codebankdepth={}".format(re.escape(directive_prefix), number_pattern)),
    }
    ...
    try:
        ...
        with open(state.inputfile, 'rt') as f:
            ...
            for line in f:
                ...
                # Tolerate spacing inside a directive. `'!codebank = 80` used to
                # match nothing at all - no error, the setting was silently
                # dropped - so the code banks fell back to their default pages
                # and landed on top of whatever the program loads there.
                original_line = _tidy_directive(original_line, directive_prefix)
                ...
                for key, pattern in directive_patterns.items():
                    ...
                    if match:
                        ...
                        #     print(f"Found OPT directive")
                            ...
                            # Re-match on original line to preserve case
                            original_match = pattern.search(original_line)
                            cmd_val = original_match.group(1) if original_match else match.group(1)
                            state.cmds_.append(cmd_val)
                            print(f"Found EXE    :  {CYAN}{cmd_val}{RESET} (command {len(state.cmds_)} queued)")
                            ...
                            print(f"Found HEADERLESS")
                            headerless = 1
                        elif key in ('codewindow', 'codewindowsize', 'codebankbase', 'codebankdepth'):
                            setattr(state, key, match.group(1))
                            print(f"Found {key.upper():<13}:  {CYAN}{match.group(1)}{RESET}")
                        elif key == 'codebankpages':
                            state.codebankpages = match.group(1).strip()
                            print(f"Found CODEBANKPAGES:  {CYAN}{state.codebankpages}{RESET}")
    ...
    print("Using HEAP   :  "+str(state.heap))   # '!heap= if given, else DEFAULT_HEAP
    print("Using OPT    :  "+str(state.optimize))   # '!opt= if given, else DEFAULT_OPTIMIZE
...
def ASMCompile():
    ...
    print(f"Assembling   :  {CYAN}{state.inputfile}{RESET}") # Use absolute path
    print(f"sjasmplus    :  Assembly compilation")
    print(f"Architechure :  {CYAN}ZX Next{RESET}")
    print(f"BUILD_DATE   :  {CYAN}{args.defines}{RESET}")
    print(f"{RESET}")

    if args.quiet == 1:
        print(f"{YELLOW}Assembling   :  {CYAN}{state.inputfile}{RESET}") # Use absolute path
    ...
    # Check if sjasmplus is available in the system PATH
    sjasmplus_cmd = 'sjasmplus'
    try:
        # Test if sjasmplus is available
        subprocess.run([sjasmplus_cmd, '--version'], capture_output=True, check=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        print(f"{RED}Error: sjasmplus not found in PATH. Please install sjasmplus.{RESET}")
        sys.exit(1)

    if state.createasm == 0:
        print(f"{YELLOW}Assembling ..... {MAGENTA}")

        # Build sjasmplus command
        asm_args = [
            sjasmplus_cmd,
            '--zxnext',
            '--fullpath',
        ...
        if args.defines is not None:
            # sjasmplus uses -D for defines
            asm_args.insert(1, f'-D{args.defines}')

        try:
            result = subprocess.run(asm_args, cwd=state.input_dir, capture_output=True, text=True)
            if result.returncode == 0:
                if args.quiet == 0:
                    print(f"{BOLD}{GREEN}YAY! Assembled OK! {RESET}")

                # Get file size for reporting
                if os.path.exists(output_bin):
                    file_size = os.path.getsize(output_bin)
                    print(f"Binary size  :  {GREEN}{file_size} bytes{RESET} ({file_size / 1024:.2f}KB)")
                else:
                    print(f"{YELLOW}Warning: Binary file not found after assembly{RESET}")
            else:
                print(f"{RED}Assembly FAILED :( {BOLD}{result.returncode}{RESET}")
                if result.stdout:
            ...
            print(f"{RED}Error during assembly: {str(e)}{RESET}")
            sys.exit(1)

    if state.createasm == 1:
        # Assembly file already exists, just copy it
        print(f"{GREEN}Assembly file already exists{RESET}")
        print(f"Assembly     :  {CYAN}{state.inputfile}{RESET}")
...
def ZXBCompile():
    ...
    if state.createasm == 0:
        ...
        if state.headerless == True or headerless == 1:
            print(f"Creating headerless binary")
            ...
            test=zxbc.main([state.inputfile, arch, '--headerless', '-W', '160', '-W', '140', '-W', '150', '-W', '170', '-W', '190', '-S', # Use state.inputfile
                            state.org, '-O', state.optimize, '-H', str(state.heap), '-M', map_output, '-o', output_bin, # Use absolute paths
            ...
            if gentape == 1:
                ...
                test=zxbc.main([state.inputfile, '-W', '160', '-W', '140', '-W', '150', '-W', '170', '-W', '190', '-S', # Use state.inputfile
                                state.org, '-O', state.optimize, '-H', str(state.heap), '-M', map_output, '-t', '-B', '-a', # Use absolute map path
                ...
                compile_args = [state.inputfile, arch, '-W', '160', '-W', '140', '-W', '150', '-W', '170', '-W', '190', # Use state.inputfile
                               '-S', state.org, '-O', state.optimize, '-H', str(state.heap), '-M', map_output,
                               '-o', output_bin,
                               '-I', LIB_DIR, '-I', SCRIPTS_DIR, '-D', 'ZXNEXT']

                # ZX Next banked code layout ('!codewindow=, '!codebank=, ...)
                for attr, flag in (('codewindow', '--code-window'),
                                   ('codewindowsize', '--code-window-size'),
                                   ('codebankbase', '--code-bank-base'),
                                   ('codebankpages', '--code-bank-pages'),
                                   ('codebankdepth', '--code-bank-depth')):
                    value = getattr(state, attr, None)
                    if value is not None:
                        compile_args.extend([flag, str(value)])
        ...
        test=zxbc.main([state.inputfile, arch, '-S', state.org, '-O', state.optimize, '-H', str(state.heap), # Use state.inputfile
    ...
    if test == 0:
        # Run map_formatter on the generated map file if it exists
        if os.path.exists(map_output):
            try:
                
                # Format the map file in place
                map_formatter.format_map_file(map_output, map_output+".patched")
            except Exception as e:
                print(f"{YELLOW}Warning: Failed to format map file: {str(e)}{RESET}")
...
def copy_with_timeout(src, dst):
    ...
    print(src)
    src_path = os.path.join(state.input_dir, src)
    if not os.path.exists(src_path):
        build_src = os.path.join(state.input_dir, "build", src)
...
def AnythingToRun():
    ...
    if master != '' and state.noemu == 0:
        print(master)
        cmd = BASE_DIR+'/Emu/Cspect/Cspect.exe -w3 -16bit -fill=0 -brk -tv -vsync -analytics -nextrom -map=build/'+filenamenoext+'.bas.map.patched -zxnext -mmc=./data/ '+master
...
def CreateBasicLoader():
    ...
    print(f"\n{YELLOW}Creating BASIC loader : {CYAN}{actual_template}{RESET}")
    ...
    try:
        ...
        print(f"{GREEN}BASIC loader : {loader_output_rel}{RESET}")
...
def RunCommand(state):
    # Run all commands queued from !exe= directives, in order
    original_cwd = os.getcwd()
    if not state.check_cmd or not state.cmds_:
        return

    total = len(state.cmds_)
    for idx, raw_cmd in enumerate(state.cmds_, start=1):
        cmd_to_run = raw_cmd.strip()

        # Substitute {file} with the output .nex name derived from the source .bas
        if '{file}' in cmd_to_run:
            cmd_to_run = cmd_to_run.replace('{file}', state.filenamenoext + '.nex')

        # If it's a shell script, invoke via bash explicitly so execute bit is not required
        if cmd_to_run.endswith('.sh') or '.sh ' in cmd_to_run:
            parts = cmd_to_run.split(None, 1)
            script_path = parts[0]
            script_args = parts[1] if len(parts) > 1 else ''
            # Resolve relative paths against input_dir
            if not os.path.isabs(script_path):
                script_path = os.path.join(state.input_dir, script_path)
            if not os.path.exists(script_path):
                print(f"{RED}Shell script not found: {script_path}{RESET}")
                continue
            cmd_to_run = f'bash "{script_path}"'
            if script_args:
                cmd_to_run += f' {script_args}'

        print(f"\n{YELLOW}Running !exe command {idx}/{total}: {CYAN}{cmd_to_run}{RESET}")
        try:
            os.chdir(state.input_dir)
            subprocess.run(cmd_to_run, shell=True, check=True,
                           stderr=subprocess.PIPE, text=True)
            os.chdir(original_cwd)
            print(f"{GREEN}Command {idx}/{total} executed successfully{RESET}")
        except subprocess.CalledProcessError as e:
            os.chdir(original_cwd)
            print(f"{RED}Command {idx}/{total} failed (exit code {e.returncode}): {cmd_to_run}{RESET}")
            if e.stderr:
                print(f"{RED}Stderr: {e.stderr.strip()}{RESET}")
        except Exception as e:
            os.chdir(original_cwd)
            print(f"{RED}Command {idx}/{total} error: {str(e)}{RESET}")

    os.chdir(original_cwd)  

def run_cspect(state):
    """Launches the CSpect emulator with appropriate arguments."""
    if state.noemu or noemu == 1:
    ...
    map_arg = f"-map={map_file_abs}"+".map"
...
def main():
    ...
    # --- NEX only (no compile) ---
    if args.nex_only:
        print(f"{MAGENTA}--- NEX Only Mode (no compile) ---{RESET}")
        build_nex_from_state()
        sys.exit(0)
    # --- End NEX Only Mode ---
    ...
    if args.runmaster == 0:
        ...
        if state.filetype == 'asm':
            ASMCompile()
        else:  # default to BASIC compilation
            ZXBCompile()

        # compiled ok, new lets generate a config to make a NEX file

        # Generate NEX config (different handling for assembly vs BASIC files)
        if state.filetype == 'asm':
            ParseNEXCfgASM()  # Use simplified config generation for assembly
        else:
            ParseNEXCfg()  # Full config parsing for BASIC files
        ...
        # Captured HERE so it times the build, not the post-build actions below
        # (an !exe of s2f can take longer than the compile).
        timetaken = str(datetime.now()-start)
        ...
        # Printed after all of the above so "All done" really is the final line -
        # CopyToDestination, sync_to_hdf and !exe commands all produce output.
        if state.module:
            print(f"{YELLOW}{BOLD}Overall build time : {timetaken[:-5]}s{RESET}")
        else:
            print(f"{BOLD}{GREEN}NEX created OK! All done.{RESET}   "
                  f"{YELLOW}{timetaken[:-5]}s{RESET}")

    if state.nonex:
        print(f"{RED}NONEX mode detected - skipping emulator launch{RESET}")
        exit(-1)        # Exit with error code if NONEX mode is detected
