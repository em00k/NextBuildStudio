...
asm    	
	jp _over_ayfx_descriptors
	; 64 bytes buffers 
	; we set this the location of these interrupt system addresses so they are available
	; from all modules. 
	; 
    afxChDesc:       	ds      8*3 ;$fd02			; fixed address of afxChDesc
    sfxenablednl:    	dw  	0 ;$fd40			; fixed address of sfxenablednl
;	currentmusicbanknl:	dw	 	0 ;$fd44			; current music banks 2 bytes 
;	currentsfxbank:		dw	 	0 ;$fd48			; current sfx bank 2 byte 
    bankbuffersplayernl: dw     0,0,0 ;$fd50			; fixed address of bankbufferplayernl
	sampletoplay:		dw	 	$00ff ;$fd58 		; low byte = sample to play, $FF = none.
													; MUST start at $FF: the ISR treats
													; anything else as a request, so a 0
													; here fires effect 0 on the very
													; first interrupt - that is the
													; mystery sound at start up.
	ayfxbankinplaycode:	dw	 	0,0 ;$fd3e
	second_mod_address:	dw	 	0,0 ;$fd60 			; address of 2nd mod if TS
	
	_over_ayfx_descriptors:
...
Sub fastcall SetUpIM()
	...
	asm 
		...
		ld      hl,IM_vector                    ; start of interrupt vector 
		ld      de,IM_vector+1
		ld      bc,257
		ld      a,h                         ; fill with IM_vector/256
		ld      i,a 
		ld      (hl),a 
		ldir 
...
Sub fastcall ISR()
	...
	asm 
		...
		jr 		z,no_sfx_to_play					; then nothing to play - but STILL run the callback
...
sub fastcall PlaySFXSys()				
	'#ifdef IM2 
	ASM 
		ret							; PlaySFXSys() is a CONTAINER, not an entry
									; point. InitSFX calls it only so the label
									; below survives dead-stripping - the ISR is
									; what actually does `call PlaySFX`.
									;
									; Without this ret the init call FALLS INTO
									; the player and fires whatever sampletoplay
									; happens to hold: 0 gave the unwanted effect
									; at start up, $FF an out-of-range index whose
									; garbage pointer lands in a channel
									; descriptor and then plays for ever.
	; ------------------------------------------------- -------------;
