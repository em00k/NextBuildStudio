...
'!HEAP=2048
'!codebank=30
'!exe=s2f copper_2voice_CODEBANK.nex
...
' CopperAudioStart()
'
' ...and this build puts the engine in a CODEBANK, so there is no bank number
' to pass, no LoadSDBank for the engine, and no hand-rolled paging in the
' stubs - the far-call trampolines do it. Two things that MUST hold:
'   '!ORG must clear the $6000-$7FFF code window (hence 32768, not 24576)
'   '!codebank=30 is the 8K page backing CODEBANK 1 - samples.bin below takes
'   31 upwards, and nextbuild.py fails the build if they ever collide.
'
' NOTE: the include has to come BEFORE the first CopperAudioLine() call -
' it is a FUNCTION, and Boriel only forward-resolves SUBs.
' entire DUAL-COPPER is now contained in it's own CODEBANK
'
#pragma codebank = 1
#include "includes/copper_include_2voice_CODEBANK.bas"
#pragma codebank = 0
...
print at 0,0;"2-voice copper - CODEBANK engine"
print at 2,0;"engine INCBIN'd into codebank 1"


' engine is in the codebank now - only the samples still need loading
LoadSDBank("samples.bin",0,0,0,$1f)                 ' sample data -> bank 31+

InitCopperAudio()                                   ' far call, run engine_init
...
print at 5,0;"sync line ";syncline
...
do
    ...
    border 2
    CopperAudioFill()                               ' far call, build one frame
    border 0
