...
sub SetUpNPC()
	...
	dim parama, paramb,a,b as ubyte 
	...
	for yy = 0 to 31 
		for xx=0 to 39
			...
			if a > $3f				
				...
				if a <$60
					AddNewSprite(cast(uinteger,xx)<<3,cast(ubyte,yy)<<3,b,a,parama,0)
				endif 
			endif 
			ccount=ccount+1
		next xx
