...
do
    ...
    border 2
    for ang = 0 to 240 step 12
    a=a+1 :
    bb=bb+1 : if bb > 63 : bb = 0 : endif

    ' Sprite 0: main circle
    position(cast(ubyte,ang)+a ,ang,120,120)
    UpdateSprite(_x,_y,0,0,0,sprX2 + sprY2)

    ' Sprite 1: smaller inner circle at different center
    position(cast(ubyte,ang)+a+64 ,ang/2,100,100)
    UpdateSprite(_x,_y,1,1,0,sprX4)

    ' Sprite 2: larger outer circle at different center
    position(cast(ubyte,ang)+a+128 ,ang + ang/2,140,140)
    UpdateSprite(_x,_y,2,2,0,sprY4)
