...
' variables need to be cast if combining = address = cast(uinteger,ubyte)*4000
' you cannot define a varible in a definitin eg : for a as ubyte = 10 to 100
...
dim ang as uinteger                          ' angle index for swirl effect
dim radius as ubyte                          ' radius growth for swirl effect
dim sxi, syi as integer                      ' signed sin/cos from table
dim dx, dy as integer                        ' computed offsets for swirl
dim cx, cy as uinteger                       ' swirl center
    ...
    do
        ...
        while screennr < 5 
            ...
            elseif scale = 192
                ...
                cls : Print at 0,0;"Swirl out effect"
                print "Sprite spirals out from center"
                print "Press any key to stop"

                ' initialise swirl parameters
                cx = 128 : cy = 96
                ang = 0 : radius = 0
                base_attr3 = 0
                base_attr4 = 0
                ...
                do
                    ' read signed sin/cos values
                    sxi = peek(@sintable + (ang band 63))
                    if sxi > 127 then sxi = sxi - 256
                    syi = peek(@sintable + ((ang + 16) band 63))
                    if syi > 127 then syi = syi - 256

                    ' grow radius and compute offsets (scale down product to fit screen)
                    if radius < 96 then radius = radius + 1
                    dx = (sxi * radius) / 2
                    dy = (syi * radius) / 2

                    ' choose scale as it moves outward
                    if radius < 16 then
                        base_attr4 = 0
                    elseif radius < 32 then
                        base_attr4 = sprX2 BOR sprY2
                    elseif radius < 64 then
                        base_attr4 = sprX4 BOR sprY4
                    else
                        base_attr4 = sprX8 BOR sprY8
                    endif

                    ' update sprite at spiral position
                    UpdateSprite(cx + dx, cy + dy, 0, plane1, base_attr3, base_attr4)
