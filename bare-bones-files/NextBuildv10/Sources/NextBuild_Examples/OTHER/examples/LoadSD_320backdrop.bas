'!ORG=24576
' LoadSDBank , 320*256BMP and FL2Text Example 
' em00k dec20

#define NEX 
#include <nextlib.bas>

border 0 

const fntbank1 as ubyte = 36			' start at bank 36 as 320x256 L2 is 10 *8kb banks 
const fntbank2 as ubyte = 37			' and starts at 24 : 24 + 10 = 34

' LoadSDBank ( filename$ , dest address, size, offset, 8k start bank )
' dest address always is 0 - 16384, this would be an offset into the banks 
' if you do not know the filesize set size to 0. If the file > 8192 the data
' is loaded into the next consecutive bank. Very handy 

LoadSDBank("[]font12.fnt",0,0,0,36) 	' load the first font to bank fntbank1 
		
LoadSDBank("[]font3.fnt",0,0,0,37) 		' load second font to fntbank2 

LoadSDBank("amiga3.bmp",0,0,1078,38) 		' loads a rotated 320*256BMP to bank 24 onwards 

InitLayer2(MODE320X256)

dim runs as ulong

do 

	CopyToBanks(38,18,10)

	FL2Text(0,31,str(runs),fntbank2)

	for y = 0 to 31
		FL2Text(10,y,"HELLO FROM NEXTBUILD",fntbank2)
		WaitRetrace(5)
	next y 

	FL2Text(11,10,"ON 320x256 LAYER 2",fntbank1)

	runs = runs + 1 
	WaitRetrace(2000)
	 
loop 
