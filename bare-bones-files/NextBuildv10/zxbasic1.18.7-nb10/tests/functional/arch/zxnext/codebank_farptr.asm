...
#line 4 "tests/functional/arch/zxnext/codebank_farptr.bas"
_fp:
	DEFB 00, 00, 00, 00
#line 5 "tests/functional/arch/zxnext/codebank_farptr.bas"
_near:
	DEFB 00
.core.ZXBASIC_USER_DATA_END:
.core.__MAIN_PROGRAM__:
	ld hl, ((.LABEL._tbl.__faraddr) + 65536) & 0xFFFF
	ld de, ((.LABEL._tbl.__faraddr) + 65536) >> 16
	ld (_fp), hl
	ld (_fp + 2), de
	ld hl, (((.LABEL._tbl.__faraddr) + 65536) + (2)) & 0xFFFF
	ld de, (((.LABEL._tbl.__faraddr) + 65536) + (2)) >> 16
	ld (_fp), hl
	ld (_fp + 2), de
	ld hl, ((_lut.__DATA__.__faraddr) + 65536) & 0xFFFF
	ld de, ((_lut.__DATA__.__faraddr) + 65536) >> 16
	ld (_fp), hl
	ld (_fp + 2), de
	ld hl, ((_lut.__DATA__.__faraddr + 3) + 65536) & 0xFFFF
	ld de, ((_lut.__DATA__.__faraddr + 3) + 65536) >> 16
	ld (_fp), hl
	ld (_fp + 2), de
	ld hl, ((_near)) & 0xFFFF
	ld de, ((_near)) >> 16
...
_Own:
		...
		CODEBANK 1
.LABEL._tbl:
.LABEL._tbl.__faraddr:
#line 12 "tests/functional/arch/zxnext/codebank_farptr.bas"
		defb 9, 8, 7, 6
#line 15 "tests/functional/arch/zxnext/codebank_farptr.bas"
_Own.__far:
	push ix
	ld ix, 0
	add ix, sp
	ld a, (_lut.__DATA__ + 0)
_Own__leave:
	ld sp, ix
	pop ix
	ret
		CODEBANK 0
	;; --- end of user code ---
#line 1 "src/lib/arch/zxnext/runtime/farcall.asm"
...
;;     _Foo:   call .core.__FAR_CALL
	...
	;; The only thing it changes is the MMU slot -- or, for a 16K window, the two
	;; adjacent slots -- covering the code window.
	;;
;; Requires (emitted by the compiler prologue):
	...
	;;
	;; With __FAR_WINDOW_16K__ defined (the compiler sets it when the code window
	;; is larger than one MMU slot) the window spans two slots, the prologue also
	;; emits .core.__FAR_MMU_REG2, and __CODE_BANK_TABLE carries two bytes per
	;; logical bank instead of one.
	;;
...
__FAR_CALL_SWITCH:
        ...
        ld   (hl), e                ; caller ret := __FAR_RETURN
	        ...
	        ld   a, c
	        ld   (__FAR_CUR_BANK), a
#line 128 "src/lib/arch/zxnext/runtime/farcall.asm"
...
#line 141 "src/lib/arch/zxnext/runtime/farcall.asm"
	        pop  bc
	        pop  de
	        pop  hl
	        pop  af
...
__FAR_RETURN:
	        ...
	        ld   (__FAR_SP), hl
	        ld   (__FAR_CUR_BANK), a
#line 175 "src/lib/arch/zxnext/runtime/farcall.asm"
	        ld   hl, __CODE_BANK_TABLE
	        add  a, l
	        ld   l, a
	        adc  a, h
	        ...
	        ld   a, (hl)
	        nextreg __FAR_MMU_REG, a
#line 188 "src/lib/arch/zxnext/runtime/farcall.asm"
	        ld   hl, 8
	        add  hl, sp                 ; HL = R-2
	        ld   (hl), c
	        inc  hl
...
#line 225 "src/lib/arch/zxnext/runtime/farcall.asm"
	        xor  a
	        ld   (__FAR_CUR_BANK), a
	        ld   hl, __FAR_STACK
	        ld   (__FAR_SP), hl
	        ret
	    pop namespace
#line 28 "tests/functional/arch/zxnext/codebank_farptr.bas"
	CODEBANK 1
#line 8 "tests/functional/arch/zxnext/codebank_farptr.bas"
_lut:
	DEFW .LABEL.__LABEL0
_lut.__DATA__.__PTR__:
	DEFW _lut.__DATA__
	DEFW 0
	DEFW 0
_lut.__DATA__.__faraddr:
_lut.__DATA__:
	DEFB 01h
	DEFB 02h
	DEFB 03h
