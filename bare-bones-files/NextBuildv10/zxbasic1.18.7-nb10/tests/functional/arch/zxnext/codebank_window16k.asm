...
#line 128 "/zxbasic/src/lib/arch/zxnext/runtime/farcall.asm"
...
#line 141 "/zxbasic/src/lib/arch/zxnext/runtime/farcall.asm"
	        pop  bc
	        pop  de
	        pop  hl
	        pop  af
...
__FAR_RETURN:
	        ...
	        ld   (__FAR_CUR_BANK), a
	        add  a, a                   ; two bytes per row in a 16K window
#line 175 "/zxbasic/src/lib/arch/zxnext/runtime/farcall.asm"
	        ld   hl, __CODE_BANK_TABLE
	        add  a, l
	        ld   l, a
	        adc  a, h
	        ...
	        ld   a, (hl)
	        nextreg __FAR_MMU_REG2, a
#line 188 "/zxbasic/src/lib/arch/zxnext/runtime/farcall.asm"
	        ld   hl, 8
	        add  hl, sp                 ; HL = R-2
	        ld   (hl), c
	        inc  hl
...
__FAR_INIT:
        ...
        ; The boot pair is read, not assumed: the stock mapping puts pages 11
	        ; and 4 at slots 3 and 4, so it is not consecutive the way a banked
	        ; pair always is.
	        dec  b                      ; BC = 0x243B again
	        ld   a, __FAR_MMU_REG2
	        out  (c), a
	        inc  b
	        in   a, (c)
	        ld   (__CODE_BANK_TABLE + 1), a
#line 225 "/zxbasic/src/lib/arch/zxnext/runtime/farcall.asm"
	        xor  a
	        ld   (__FAR_CUR_BANK), a
	        ld   hl, __FAR_STACK
	        ld   (__FAR_SP), hl
	        ret
	    pop namespace
#line 46 "arch/zxnext/codebank_window16k.bas"
__EXIT_FUNCTION:
	ld sp, ix
	pop ix
	pop de
