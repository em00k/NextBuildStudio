...
# --- NextBuild fork warnings -------------------------------------------------
# Codes 900+ are reserved for this fork. Upstream numbers its own from 100 and
# has reached 300, so anything in the 3xx range we take is standing where its
# next warning will land -- which is exactly what happened to the 310/320 these
# two used to use. Never register a fork warning below 900. See
# codebanks-fork/SYNCING.md.


@register_warning("900")
def warning_byref_leaves_codebank(lineno: int, id_: str, bank: int, callee: str, fname: str | None = None):
    warning(
        lineno,
        f"'{id_}' lives in CODEBANK {bank} but is passed ByRef to '{callee}', which does not. "
        f"The callee gets a plain address that stops being valid as soon as the bank is paged out",
        fname=fname,
    )


@register_warning("910")
def warning_codebank_has_no_routines(lineno: int, bank: int, fname: str | None = None):
    warning(
        lineno,
        f"CODEBANK {bank} holds data but no SUB or FUNCTION, so nothing ever pages it in. "
        f"Bank-local data is only reachable from routines in the same bank",
        fname=fname,
    )


@register_warning("920")
def warning_addressof_array(lineno: int, id_: str, subscripts: str, fname: str | None = None):
    """`@arr` is the array descriptor, not its data.

    Upstream changed this in 1.18.3 (commit 4b8832a1): `@arr` used to give the
    address of the first element and now gives the address of the descriptor --
    a pointer to the dimension table, then the data pointer, then the bound
    pointers. Code that walks it with PEEK reads that header instead of the
    data, silently, which is how a sprite table came out as one sprite repeated.
    """
    warning(
        lineno,
        f"'@{id_}' is the address of the array descriptor, not its data. Since 1.18.3 write "
        f"'@{id_}({subscripts})' for the address of the first element",
        fname=fname,
    )
