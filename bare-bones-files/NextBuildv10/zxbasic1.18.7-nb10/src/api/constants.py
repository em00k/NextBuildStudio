...
class TYPE(enum.IntEnum):
    ...
    @classmethod
    def bounds(cls, type_: "TYPE") -> Optional[tuple[int, int]]:
        """Returns the (min, max) values an integral type can hold.
        Returns None for non-integral types (fixed, float, string, unknown).
        """
        if type_ not in cls.integral:
            return None

        bits = 8 * cls.type_size(type_)
        if cls.is_unsigned(type_):
            return 0, (1 << bits) - 1
