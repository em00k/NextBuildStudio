...
from src.api.codebank_options import current_codebank
...
class SymbolTable:
    ...
    def __init__(self):
        ...
        self.basic_types = {}
        # ZX Next: set while a FARPTR is being parsed. Naming a bank-local
        # symbol is normally an error, and FARPTR is the one sanctioned way to
        # do it -- see far_access() below.
        self._far_access = False
    ...
    def far_access(self):
        """Suspends the cross-bank check for the symbol accesses made inside.

        `FARPTR x` deliberately names a bank-local symbol from outside its bank,
        and the access funnels down through access_array/make_array_access, several
        layers below the grammar rule. Rather than thread a flag through all of
        them, the rule brackets the access with this.

        What the pointer it produces is allowed to *do* is enforced elsewhere:
        it names a FAR_ADDR_SUFFIX alias, and only the far-memory runtime, which
        pages the bank in first, ever dereferences one.
        """
        prev = self._far_access
        self._far_access = True
        try:
            yield
        finally:
            self._far_access = prev
    ...
    def get_entry(self, id_: str, scope: Scope | None = None, lineno: int | None = None) -> symbols.ID | None:
        ...
        if entry is not None and suffix is not None and lineno is not None:
            if isinstance(entry, symbols.ID) and entry.type_ is not None:
                expected_type = symbols.TYPEREF(self.basic_types[SUFFIX_TYPE[suffix]], lineno)
                if entry.type_ != expected_type:
                    if suffix == "$":
                        syntax_error(lineno, "Invalid $ suffix (or sigil) for numeric variable")
                    else:
                        syntax_error(lineno, f"Invalid {suffix} suffix (or sigil) for {entry.type_.name} variable")

        return entry

    def get_existing_entry(self, id_: str, scope: Scope | None = None, lineno: int | None = None) -> symbols.ID:
        result = self.get_entry(id_, scope, lineno=lineno)
    ...
    def check_class(self, id_: str, class_: CLASS, lineno: int, scope: Scope | None = None, show_error=True) -> bool:
        ...
        entry = self.get_entry(id_, scope, lineno=lineno)
    ...
    def access_id(
        ...
        # Every way of naming a variable funnels through here -- access_var and
        # access_array both delegate to it, and a bare ID in an expression comes
        # straight in (p_id_expr) -- so this is the one place the cross-bank
        # check has to live to be seen by all of them.
        self._check_codebank_access(result, lineno)
        return result
    ...
    def declare_variable(self, id_: str, lineno: int, type_, default_value=None, class_: CLASS = CLASS.var):
        ...
        if not self.check_is_undeclared(id_, lineno, scope=self.current_scope, show_error=False):
            entry_ = self.get_existing_entry(id_, lineno=lineno)
        ...
        entry = self.get_entry(id_, scope=self.current_scope, lineno=lineno)
        if entry is None:
        ...
        self._bind_to_codebank(entry)
        return entry

    def _bind_to_codebank(self, entry: symbols.ID) -> None:
        """ZX Next: places a global inside the code bank currently in effect.

        A variable or array declared between `CODEBANK n` and `END CODEBANK`
        (or under `#pragma codebank = n`) has its storage emitted into bank n's
        8K page, next to the routines that use it, instead of into the resident
        variable area. It is then only addressable while that bank is paged in,
        so only routines in the same bank may touch it.

        Locals and parameters live on the IX frame and are always resident, and
        CONSTs never get storage at all, so neither is bound.
        """
        if entry is None or entry.class_ not in (CLASS.var, CLASS.array):
            return

        if self.current_scope == self.global_scope:
            entry.ref.bank = current_codebank()

    @staticmethod
    def _codebank_in_scope() -> int:
        """Code bank the statement currently being parsed will be compiled into.

        Inside a SUB/FUNCTION that is whatever bank the routine was bound to.
        At module level we use the bank currently in effect rather than 0,
        because that is also where declarations are reduced -- a `DIM` between
        `CODEBANK 1` and `END CODEBANK` must not report itself as a cross-bank
        access. Module-level *statements* written in the same place do compile
        into the resident main body, so those slip past this check and are
        caught by the assembler instead.
        """
        if global_.FUNCTION_LEVEL:
            return int(getattr(global_.FUNCTION_LEVEL[-1].ref, "bank", 0) or 0)

        return current_codebank()

    def _check_codebank_access(self, entry, lineno: int) -> None:
        """Errors if a bank-local variable is reached from outside its own bank.

        Only one code bank is paged into the code window at a time, so a
        bank-local variable is addressable only from routines in that same bank.
        Reported here purely to get a BASIC line number on it; the assembler's
        cross-segment check is what actually guarantees the rule, and covers the
        routes that never come through here (@address, ByRef, whole-array copy).

        Restricted to variables and arrays. A banked SUB/FUNCTION carries a bank
        on its ref too, but calling one from anywhere is the whole point of the
        resident trampoline.
        """
        if self._far_access:
            return  # FARPTR: naming a bank-local symbol is exactly the point

        if entry is None or entry.class_ not in (CLASS.var, CLASS.array):
            return

        bank = int(getattr(entry.ref, "bank", 0) or 0)
        if not bank or bank == self._codebank_in_scope():
            return

        where = "CODEBANK %i" % self._codebank_in_scope() if self._codebank_in_scope() else "the resident program"
        syntax_error(
            lineno,
            "'%s' lives in CODEBANK %i and cannot be reached from %s. Bank-local data is only addressable "
            "while its own bank is paged in" % (entry.name, bank, where),
        )
    ...
    def declare_const(self, id_: str, lineno: int, type_, default_value):
        ...
        if not self.check_is_undeclared(id_, lineno, scope=self.current_scope, show_error=False):
            entry = self.get_entry(id_, lineno=lineno)
    ...
    def declare_label(self, id_: str, lineno: int) -> symbols.ID | None:
        ...
        if not self.check_is_undeclared(id_, lineno, "label"):
            e = self.get_existing_entry(id_, lineno=lineno)
        ...
        entry = (
            self.get_entry(id_, scope=self.current_scope, lineno=lineno)
            or self.get_entry(id_, scope=self.global_scope, lineno=lineno)
    ...
    def declare_array(self, id_: str, lineno: int, type_, bounds, default_value=None, addr=None):
        ...
        entry = self.get_entry(id_, self.current_scope, lineno=lineno)
        if entry is None:
            entry = self.declare(id_, lineno, symbols.ID(name=id_, lineno=lineno, type_=type_))
            assert entry is not None
    ...
    def declare_func(self, id_: str, lineno: int, type_=None, class_=CLASS.function) -> symbols.ID | None:
        ...
        entry = self.get_entry(id_, lineno=lineno)  # Must not exist or have CLASS.unknown/function undeclared
