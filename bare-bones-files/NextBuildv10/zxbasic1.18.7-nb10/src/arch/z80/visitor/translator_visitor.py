...
from src.arch.z80.backend.codebank import far_alias
...
class TranslatorVisitor(TranslatorInstVisitor):
    ...
    def traverse_const(node):
        ...
        if node.token == "UNARY":
            ...
            if mid == "MINUS":
                ...
                return TranslatorVisitor.traverse_far_address(node.operand)
            elif mid == "ADDRESS":
    ...
    @staticmethod
    def traverse_far_address(node) -> str | None:
        """`FARPTR x` as an assembler expression: (bank << 16) | address.

        The bank is read here rather than at parse time because a module-level
        asm label is not bound to its bank until bind_module_level_asm has run.

        The label named is never the real one. A bank-local symbol gets a
        FAR_ADDR_SUFFIX alias emitted beside its storage, and this names that,
        so the resident program holds no raw pointer into a bank as far as
        Memory.check_cross_segment_refs is concerned. A resident symbol needs
        no alias and yields bank 0, which is correct rather than merely
        tolerated: the far accessors then map the boot page (a no-op) and read
        an address that was never in the window to begin with.
        """
        if node.token == "ARRAYACCESS":
            entry = node.entry
            base = f"{far_alias(entry, entry.data_label)} + {node.offset}"
        else:
            entry = node
            name = entry.data_label if entry.class_ == CLASS.array else entry.t
            base = far_alias(entry, name)

        bank = codebank.bank_of_var(entry)
        if not bank:
            return f"({base})"

        return f"({base}) + {bank * 0x10000}"
