...
class Translator(TranslatorVisitor):
    ...
    def visit_LABEL(self, node):
        # ZX Next: a label naming a diverted asm block moves into the bank with it
        if codebank.collect_if_diverted(node):
            return
    ...
    def _check_byref_codebank(self, node) -> None:
        """Warns when a bank-local variable is passed ByRef out of its own bank.

        ByRef hands the callee a bare 16-bit address. That address is in the
        code window, so it only means anything while the variable's bank is
        paged in -- and a resident callee runs after __FAR_RETURN has already
        restored the previous page. Neither the front-end check nor the
        assembler's cross-segment check can see this one, because the `ld hl,
        #_var` is emitted from inside the bank, where it is perfectly legal.
        """
        callee_bank = codebank.bank_of(node.entry)

        for arg in node.args:
            if not getattr(arg, "byref", False):
                continue

            value = arg.value
            entry = getattr(value, "entry", value)
            bank = codebank.bank_of_var(entry)
            if bank and bank != callee_bank:
                src.api.errmsg.warning_byref_leaves_codebank(node.lineno, entry.name, bank, node.entry.name)

    def visit_CALL(self, node: symbols.CALL):
        self._check_byref_codebank(node)
    ...
    @staticmethod
    def _for_wraps_around(node) -> tuple[bool, int | None]:
        """Whether a FOR loop's "var > limit" (or "var < limit") exit test can never
        fire, because adding STEP to the iterator wraps around the type before the
        iterator can ever pass the limit. E.g. FOR n as UBYTE = 0 TO 255.

        Only provable for a constant limit and a constant step.

        Returns (wraps, guard), where guard is the constant the iterator must be
        compared against *before* incrementing it. A guard of None means the loop
        can never run more than one iteration.
        """
        limit_, step_ = node.children[2], node.children[3]

        if not check.is_number(limit_, step_) or not step_.value:
            return False, None

        type_ = node.children[0].type_
        if not type_.is_basic:
            return False, None

        bounds = TYPE.bounds(type_.type_)
        if bounds is None:  # not an integral type, so it does not wrap
            return False, None

        lo, hi = bounds
        if not lo <= limit_.value + step_.value <= hi:
            # var can never pass limit_ - test var against (limit_ - step_) instead,
            # which cannot wrap because var <= limit_ holds when it is evaluated
            guard = limit_.value - step_.value
            return True, guard if lo <= guard <= hi else None
    ...
    def visit_FOR(self, node):
        ...
        wraps, guard = self._for_wraps_around(node)
        if wraps:
            yield from self._emit_wrap_safe_for(node, guard, end_loop, loop_body, loop_continue)
            self.LOOPS.pop()
            return
    ...
    def _emit_wrap_safe_for(self, node, guard, end_loop, loop_body, loop_continue):
        """Emits a FOR loop whose iterator would wrap around before it could pass
        the limit (see _for_wraps_around). The exit test is moved *ahead* of the
        increment and compared against `guard` == limit - step, so neither the
        test nor the increment can overflow:

            var = start
            if var > limit -> end_loop          ; positive step ("<" if negative)
        body:
            <body>
        continue:
            if var > guard -> end_loop          ; i.e. var + step would pass limit
            var = var + step
            jp body
        end_loop:
        """
        type_ = node.children[0].type_
        ascending = node.children[3].value > 0
        ic_cmp = self.ic_gt if ascending else self.ic_lt

        yield node.children[1]  # Starting value
        self.emit_let_left_part(node)  # Stores it in the iterator variable

        # Entry test: skip the loop entirely if it starts out past the limit
        yield node.children[0]  # Iterator var
        yield node.children[2]  # Limit
        t = optemps.new_t()
        ic_cmp(type_, t, node.children[0].t, node.children[2].t)
        self.ic_jnzero(TYPE.ubyte, t, end_loop)
        ...
        if guard is None:
            # limit - step is not representable, so a 2nd iteration is impossible:
            # fall straight through to end_loop
            self.ic_label(end_loop)
            return

        # Exit *before* incrementing, so the iterator never wraps
        guard_ = symbols.NUMBER(guard, lineno=node.lineno, type_=type_)
        yield node.children[0]  # Iterator var
        yield guard_
        t = optemps.new_t()
        ic_cmp(type_, t, node.children[0].t, guard_.t)
        self.ic_jnzero(TYPE.ubyte, t, end_loop)
    ...
    # -----------------------------------------------------------------------------------------------------
    def emit_asm_node(self, node) -> None:
        """Emits an inline asm block, wrapped in the #line markers it needs.

        Split out of visit_ASM so that FunctionTranslator can replay a block
        diverted into a code bank without duplicating the marker arithmetic.
        """
        EOL = "\n"
    ...
    def visit_ASM(self, node):
        # ZX Next: a module-level block inside a CODEBANK block belongs in that
        # bank, not here. It is parked now and replayed inside the bank's fence
        if codebank.collect_if_diverted(node):
            return
