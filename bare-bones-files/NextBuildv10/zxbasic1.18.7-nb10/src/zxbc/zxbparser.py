...
from src.zxbc import keywords, zxblex
...
# A line of inline asm consisting of nothing but the assembler's own CODEBANK
# directive. The lexer has already stripped `;` comments from the block.
_RE_ASM_CODEBANK = re.compile(r"^\s*CODEBANK\b\s*(.*?)\s*$", re.IGNORECASE)


def _codebank_name(bank: int) -> str:
    return "CODEBANK %i" % bank if bank else "the resident program"


def _check_asm_codebank_balance(asm: str, lineno: int, enclosing: int) -> None:
    """ZX Next: an asm block must leave the assembler in the segment it started in.

    The assembler's `CODEBANK` directive may be used inside an asm block to
    place a blob in a bank by hand, which is a supported escape hatch. But the
    block is emitted in the middle of a stream the compiler is fencing itself,
    so one that switches segment and does not switch back silently misplaces
    everything emitted after it.

    Purely textual, so a directive hidden behind `#if` or produced by a macro is
    invisible here; a non-literal bank number gives up rather than guessing.
    """
    current = enclosing

    for line in asm.split("\n"):
        match = _RE_ASM_CODEBANK.match(line)
        if match is None:
            continue

        if not match.group(1).isdecimal():
            return  # Not a plain number. We cannot follow it, so say nothing
    ...
    error(
        lineno,
        "This 'asm' block is assembled into %s but its 'CODEBANK' directives leave the assembler in %s. "
        "It must switch back before it ends" % (_codebank_name(enclosing), _codebank_name(current)),
    )
...
def make_asm_sentence(asm: str, lineno: int, sentinel: bool = False):
    ...
    ZX Next: a block written at module level inside a `CODEBANK n` block is
    compiled into bank n rather than run inline. That can only be decided here,
    because OPTIONS.codebank is long back to 0 by the time the AST is walked.
    FUNCTION_LEVEL gates it: a block inside a banked SUB already reaches the
    bank through the routine's body and must not be moved a second time.

    Which of these blocks are *really* diverted is settled after parsing, by
    zxbc.codebank_asm -- only a direct child of the top-level statement list
    qualifies, so one nested in an IF or a FOR keeps running where it stands.
    """
    enclosing = int(OPTIONS.codebank or 0)
    _check_asm_codebank_balance(asm, lineno, enclosing)

    bank = 0 if (sentinel or FUNCTION_LEVEL) else enclosing
    return sym.ASM(asm, lineno, gl.FILENAME, is_sentinel=sentinel, bank=bank)
...
def p_var_decl_ini(p):
    ...
    if defval is None:  # Okay do a delayed initialization
        entry = SYMBOL_TABLE.access_var(idlist[0].name, p.lineno(1))
        p[0] = _delayed_init(p.lineno(1), entry, make_sentence(p.lineno(1), "LET", entry, value))


def _delayed_init(lineno: int, entry, sentence):
    """Wraps a delayed `DIM x = value` initialisation so it runs in x's bank.

    Only a *static* initialiser becomes storage; everything else -- every
    String, and any expression that is not a compile-time constant -- is
    deferred into a LET that Boriel drops into the enclosing statement list. At
    module level that list is the resident main body, even between `CODEBANK n`
    and `END CODEBANK`, so the store would be resident code writing to a
    bank-local variable: rejected by the assembler, and rightly so.

    Rather than let resident code reach into a bank, put the store where it
    belongs. The LET goes into a synthesised parameterless SUB bound to that
    bank, and a call to it takes the LET's place -- so it runs through the
    ordinary trampoline, with the bank paged in, and every existing rule holds
    unchanged. One SUB per initialiser rather than one per bank, so the order of
    the initialisations is exactly the order they were written.

    Anything resident is returned untouched, which keeps ordinary compilation
    byte for byte as it was.
    """
    if entry is None or gl.FUNCTION_LEVEL:
        return sentence  # a local lives on the stack, and is always resident

    bank = int(getattr(entry.ref, "bank", 0) or 0)
    if not bank:
        return sentence

    name = f"__BANKINIT__{len(gl.FUNCTIONS_BANKINIT)}"
    funcdecl = make_func_declaration(name, lineno, CLASS.sub)
    if funcdecl is None:
        return sentence

    func = funcdecl.entry
    func.ref.convention = CONVENTION.stdcall
    func.ref.bank = bank

    SYMBOL_TABLE.enter_scope(name)
    funcdecl.append_child(make_param_list(None))
    funcdecl.params_size = 0
    funcdecl.local_symbol_table = SYMBOL_TABLE.current_scope
    funcdecl.locals_size = SYMBOL_TABLE.leave_scope()
    funcdecl.local_symbol_table.owner = funcdecl
    func.ref.body = make_block(sentence)
    func.ref.forwarded = False

    gl.FUNCTIONS_BANKINIT.append(func)
    return make_block(funcdecl, make_sub_call(name, lineno, make_arg_list(None)))
...
def p_addr_of_id(p):
    ...
    # Upstream 1.18.3 (4b8832a1) changed `@arr` from the address of the first
    # element to the address of the descriptor, silently. Say so, and name the
    # replacement with the array's real arity -- `@a(0)` for one dimension,
    # `@a(0,0)` for two -- since that is the part people get wrong.
    if entry.class_ == CLASS.array and entry.scope == SCOPE.global_:
        errmsg.warning_addressof_array(p.lineno(1), entry.name, ",".join("0" * len(entry.bounds)))
...
def _make_faraddress(lineno: int, entry, operand=None):
    """ZX Next: builds the FARADDRESS node behind `FARPTR x`.

    The value is `(bank << 16) | address`, so one uLong carries everything the
    far-memory runtime needs. The bank is *not* resolved here: a module-level
    asm label is not bound to its bank until bind_module_level_asm runs, long
    after parsing. FARADDRESS therefore survives into the backend, where
    TranslatorVisitor.traverse_far_address reads the settled bank.

    `operand` is the node to hang the address off when it is not the entry
    itself -- an ARRAYACCESS, for `FARPTR a(3)`.
    """
    if entry is None:
        return None
    ...
    if entry.class_ in (CLASS.function, CLASS.sub):
        error(
            lineno,
            "Cannot take FARPTR of '%s'. A banked SUB or FUNCTION is called through its resident "
            "trampoline, which is what its ordinary name already is" % entry.name,
        )
        return None

    entry.ref.has_faraddress = True
    entry.has_address = True
    mark_entry_as_accessed(entry)

    result = make_unary(lineno, "FARADDRESS", entry if operand is None else operand, type_=TYPE.ulong)
    return make_constexpr(lineno, result)


def p_farptr_of_id(p):
    """bexpr : FARPTR singleid"""
    id_: Id = p[2]
    with SYMBOL_TABLE.far_access():
        entry = SYMBOL_TABLE.access_id(id_.name, id_.lineno, ignore_explicit_flag=True)

    if entry is None or is_dynamic(entry):
        if entry is not None:
            error(p.lineno(1), "Cannot take FARPTR of '%s'. Only a global has a fixed address" % entry.name)
        p[0] = None
        return

    p[0] = _make_faraddress(p.lineno(1), entry)


def p_farptr_of_array_element(p):
    """bexpr : FARPTR ARRAY_ID arg_list"""
    p[0] = None
    ...
    if result.offset is None:
        error(
            p.lineno(1),
            "Cannot take FARPTR of '%s(...)' with a variable subscript. The address must be known at "
            "compile time" % p[2],
        )
        return

    p[0] = _make_faraddress(p.lineno(1), result.entry, operand=result)


def p_err_farptr_undefined_arr(p):
    """bexpr : FARPTR ID arg_list"""
...
def p_function_def(p):
    ...
    if entry.class_ in (CLASS.function, CLASS.sub):  # Was correctly declared?
        ...
        # ZX Next: bind this routine to the code bank currently in effect. This
        # rule reduces on the SUB/FUNCTION *header*, before the body is parsed,
        # so a `#pragma codebank` inside the body cannot alter it. A forward
        # DECLARE and the definition share one FuncRef, so the definition wins.
...
def p_codebank_begin(p):
    ...
    | CODEBANK INTEGER
    """
    # ZX Next: every SUB/FUNCTION declared from here on goes into code bank N,
    # until the matching END CODEBANK. Nesting works because Option carries its
    # own value stack.
    if FUNCTION_LEVEL:
        errmsg.error(p.lineno(1), "CODEBANK cannot be used inside a SUB or FUNCTION")
        return

    bank = p[2]
    if bank != int(bank) or bank < 0:
        errmsg.error(p.lineno(1), "Invalid CODEBANK number '%s'. Must be a whole number, 0 or greater" % bank)
        return
...
def p_codebank_end(p):
    ...
    except src.api.options.OptionStackUnderflowError:
        errmsg.error(p.lineno(1), "END CODEBANK without a matching CODEBANK")
...
# Initialization
# ----------------------------------------
parser = src.api.utils.get_or_create(
    "zxbparser",
    lambda: yacc.yacc(**src.api.utils.ply_yacc_kwargs()),
    depends_on=(__file__, zxblex.__file__, keywords.__file__),
)
