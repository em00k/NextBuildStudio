...
def parse_options(args: list[str] | None = None) -> Namespace:
    ...
    # ZX Next banked code layout. A `#pragma` in the source overrides these,
    # since pragmas are applied later, while the program is being parsed.
    for opt_name, arg_name in (
        ("codewindow", "code_window"),
        ("codewindowsize", "code_window_size"),
        ("codebankbase", "code_bank_base"),
        ("codebankdepth", "code_bank_depth"),
    ):
        arg = getattr(options, arg_name, None)
        if arg is None:
            continue
        value = src.api.utils.parse_int(arg)
        if value is None:
            parser.error(f"Invalid --{arg_name.replace('_', '-')} option '{arg}'")
        setattr(OPTIONS, opt_name, value)

    if getattr(options, "code_bank_pages", None) is not None:
        OPTIONS.codebankpages = options.code_bank_pages
...
def set_option_defines() -> None:
    ...
    # ZX Next: a 16K code window spans two MMU slots, so the far-call runtime
    # has to map and restore both. Set here rather than when the option is
    # parsed, because `#pragma codewindowsize` is applied while the source is
    # being parsed and this runs afterwards, just before the assembly is
    # preprocessed.
    if OPTIONS.codewindowsize > 8192:
        OPTIONS.__DEFINES["__FAR_WINDOW_16K__"] = ""
