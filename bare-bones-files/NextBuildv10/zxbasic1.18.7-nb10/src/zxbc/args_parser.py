...
def parser() -> argparse.ArgumentParser:
    ...
    parser_.add_argument(
        "--code-window",
        type=str,
        help="ZX Next: address of the window banked code is paged into. Default $6000",
    )
    parser_.add_argument(
        "--code-window-size",
        type=str,
        help="ZX Next: size of the code window, 8192 (one MMU slot) or 16384 (two). Default 8192",
    )
    parser_.add_argument(
        "--code-bank-base",
        type=str,
        help="ZX Next: physical 8K page backing CODEBANK 1. Later banks follow on. Default 30",
    )
    parser_.add_argument(
        "--code-bank-pages",
        type=str,
        help="ZX Next: explicit physical 8K pages for the code banks, e.g. 30,31,34. Overrides --code-bank-base",
    )
    parser_.add_argument(
        "--code-bank-depth",
        type=str,
        help="ZX Next: maximum cross-bank call nesting depth. Default 16",
    )
    parser_.add_argument(
