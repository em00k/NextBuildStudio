...
from src.api.codebank_options import init_codebank_options
...
def init():
    ...
    # Registered here as well as in the backend, so that the CODEBANK directive
    # also works when zxbasm is driven standalone.
...
def p_org(p):
    ...
    | ORG pexpr
    """
    if MEMORY.current_segment:
        error(p.lineno(1), "ORG is not allowed inside a CODEBANK. The bank is fixed at the code window address")
        return
...
def _routines_in_segment(segment_id: int, limit: int = 12) -> str:
    """The routines in a given code bank, largest first, as a readable summary.

    Used to make a bank-overflow message actionable: the biggest offenders are
    the ones worth moving.
    """
    labels = sorted(
        (
            lbl
            for lbl in MEMORY.global_labels.values()
            if getattr(lbl, "segment", 0) == segment_id
            and lbl.is_address
            and lbl.defined
            and not lbl.name.endswith("__leave")
        ),
    ...
    _, end = MEMORY.dump(segment_id)
    top = labels[0].value + len(end)
    sizes = [
        (labels[i].name.lstrip("."), (labels[i + 1].value if i + 1 < len(labels) else top) - labels[i].value)
        for i in range(len(labels))
    ]
    sizes.sort(key=lambda x: -x[1])

    shown = ", ".join(f"{name} ({size} bytes)" for name, size in sizes[:limit])
    if len(sizes) > limit:
        shown += f", and {len(sizes) - limit} more"
    return shown


def generate_code_banks(outputfname: str, resident_org: int, resident_len: int) -> list[dict]:
    """Writes each ZX Next code bank to its own binary alongside the main output,
    plus a JSON manifest describing them.

    Returns the manifest entries. Emits an error for a bank that overflows the
    code window, or for a code window that collides with the resident program.
    """
    from src.api.codebank_options import physical_page, window_slots

    segments = MEMORY.dump_all()
    banks = {seg_id: data for seg_id, data in segments.items() if seg_id != 0 and data[1]}
    if not banks:
        return []

    window_start = OPTIONS.codewindow
    window_end = window_start + OPTIONS.codewindowsize
    resident_end = resident_org + resident_len
    if resident_org < window_end and window_start < resident_end:
        error(
            0,
            "The code window ($%04X-$%04X) overlaps the resident program ($%04X-$%04X). "
            "Move the program ORG or the code window" % (window_start, window_end - 1, resident_org, resident_end - 1),
        )
        return []

    base, _ = os.path.splitext(outputfname)
    manifest = []

    for seg_id, (seg_org, data) in sorted(banks.items()):
        if len(data) > OPTIONS.codewindowsize:
            error(
                0,
                "CODEBANK %i is %i bytes, %i over the %i byte code window. Largest routines: %s"
                % (
                    seg_id,
                    len(data),
                    len(data) - OPTIONS.codewindowsize,
                    OPTIONS.codewindowsize,
                    _routines_in_segment(seg_id),
                ),
            )
            continue

        fname = "%s.bank%i.bin" % (base, seg_id)
        with src.api.utils.open_file(fname, "wb") as f:
            f.write(bytearray(data))

        # `page` stays the first page, which is all a loader needs -- a blob is
        # written from there and spills into the pages that follow. `pages`
        # spells out the whole allocation, so the packer can see that a 16K
        # bank reserves two of them.
        first_page = physical_page(seg_id)
        manifest.append(
            {
                "bank": seg_id,
                ...
                "size": len(data),
                "page": first_page,
                "pages": [first_page + i for i in range(window_slots())],
            }
        )

    if manifest:
        with src.api.utils.open_file("%s.banks.json" % base, "wt", "utf-8") as f:
            json.dump({"window": window_start, "window_size": OPTIONS.codewindowsize, "banks": manifest}, f, indent=2)
...
def generate_binary(outputfname, format_, progname="", binary_files=None, headless_binary_files=None, emitter=None):
    ...
    MEMORY.check_undefined_labels()
    MEMORY.check_cross_segment_refs()

    org, binary = MEMORY.dump(0)
    if gl.has_errors:
        return

    generate_code_banks(outputfname, org, len(binary))
    if gl.has_errors:
        return
...
# The sources each cached parser table is derived from. Change one and the
# cache entry is discarded rather than silently reused -- see utils.get_or_create.
_GRAMMAR_SOURCES = (__file__, asmlex.__file__)

# Z80 only ASM parser
parser = src.api.utils.get_or_create(
    "asmparse", lambda: yacc.yacc(start="start", **src.api.utils.ply_yacc_kwargs()), depends_on=_GRAMMAR_SOURCES
)

# needed for ply
from .zxnext import *  # noqa

# ZXNEXT extended Opcodes parser. Named by path rather than through an import,
# which the star import above has deliberately not bound.
zxnext_parser = src.api.utils.get_or_create(
    "zxnext_asmparse",
    lambda: yacc.yacc(start="start", **src.api.utils.ply_yacc_kwargs()),
    depends_on=_GRAMMAR_SOURCES + (os.path.join(os.path.dirname(__file__), "zxnext.py"),),
)
